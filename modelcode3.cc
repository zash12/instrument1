...
struct RoutingStats {
    ...
    // ── SendOutgoing: locally ORIGINATED packets only ─────────
    uint64_t orig_totalTx = 0;  // all originated
    uint64_t orig_dataTx  = 0;  // originated data (port 9)
    uint64_t orig_ctrlTx  = 0;  // originated control (non port-9)
    uint64_t orig_totalB  = 0;
    uint64_t orig_ctrlB   = 0;

    // ── UnicastForward: FORWARDED packets only ────────────────
    uint64_t fwd_totalTx  = 0;  // all forwarded
    uint64_t fwd_dataTx   = 0;  // forwarded data (port 9)
    uint64_t fwd_ctrlTx   = 0;  // forwarded control
    uint64_t fwd_totalB   = 0;
    uint64_t fwd_ctrlB    = 0;
};
...
// ── SendOutgoing on ALL nodes: ORIGINATED packets only ───────
static void Ipv4SendOutgoingCb (const Ipv4Header &iph,
                                 Ptr<const Packet> pkt,
                                 uint32_t /*iface*/)
{
    uint32_t sz = pkt->GetSize () + iph.GetSerializedSize ();
    g_rstat.orig_totalTx++;
    g_rstat.orig_totalB += sz;

    bool isData = false;
    if (iph.GetProtocol () == 17) {
        Ptr<Packet> cp = pkt->Copy ();
        UdpHeader udph;
        if (cp->GetSize () >= 8) {
            cp->PeekHeader (udph);
    ...
    if (isData) {
        ...
        g_rstat.orig_ctrlTx++;
        g_rstat.orig_ctrlB += sz;
    }
}

// ── UnicastForward on ALL nodes: FORWARDED packets only ──────
static void Ipv4UnicastForwardCb (const Ipv4Header &iph,
                                   Ptr<const Packet> pkt,
                                   uint32_t /*iface*/)
{
    uint32_t sz = pkt->GetSize () + iph.GetSerializedSize ();
    g_rstat.fwd_totalTx++;
    g_rstat.fwd_totalB += sz;
                     ...
                     std::ofstream &appLog, std::ofstream &routingLog)
{
    ...
    // LocalDeliver at sink + SendOutgoing + UnicastForward on ALL nodes
    for (uint32_t i = 0; i < (uint32_t)numNodes; i++) {
        ...
        ip->TraceConnectWithoutContext (
            "UnicastForward", MakeCallback (&Ipv4UnicastForwardCb));
        if (i == dstIdx)
            ip->TraceConnectWithoutContext (
    ...
    // ==========================================================
    {
        ...
        // ── NRL definitions ───────────────────────────────────
        // NRL1: standard = originated ctrl pkts / rx data pkts
        //       (broadcasts included, no forwarded data counted)
        double nrl1 = rx.rxPkts > 0
                      ? (double)g_rstat.orig_ctrlTx / rx.rxPkts : 0.0;

        // NRL2: total network load = (orig+fwd ctrl) / rx data pkts
        double nrl2 = rx.rxPkts > 0
                      ? (double)(g_rstat.orig_ctrlTx + g_rstat.fwd_ctrlTx)
                        / rx.rxPkts : 0.0;

        // NRL3: full network tx / rx data (includes data forwards)
        uint64_t totalAllTx = g_rstat.orig_totalTx + g_rstat.fwd_totalTx;
        double nrl3 = rx.rxPkts > 0
                      ? (double)totalAllTx / rx.rxPkts : 0.0;

        // Overhead % of total originated bytes
        double rohPct_orig = g_rstat.orig_totalB > 0
                             ? 100.0*g_rstat.orig_ctrlB/g_rstat.orig_totalB
                             : 0.0;
        // Overhead % including forwarded traffic
        uint64_t totalB = g_rstat.orig_totalB + g_rstat.fwd_totalB;
        uint64_t ctrlB  = g_rstat.orig_ctrlB  + g_rstat.fwd_ctrlB;
        double rohPct_all = totalB > 0 ? 100.0*ctrlB/totalB : 0.0;
        ...
        std::cout << "\n  [Routing]"
                  ...
                  << " avgTTL="      << avgTTL
                  << " RDT="         << rdt_ms       << "ms"
                  << "\n  [NRL1-orig_ctrl_only]="  << nrl1
                  << "  [NRL2-all_ctrl]="          << nrl2
                  << "  [NRL3-all_tx]="            << nrl3
                  << "\n  [OverheadPct-orig]="     << rohPct_orig
                  << "%  [OverheadPct-all]="       << rohPct_all << "%\n";
...
int main (int argc, char *argv[])
{
    ...
    routingLog << "RunID,NumNodes,Gap,"
                  "AvgHop,MinHop,MaxHop,AvgTTL,RDT_ms,"
                  "OrigCtrlTx,FwdCtrlTx,TotalCtrlTx,"
                  "NRL1_origCtrl,NRL2_allCtrl,NRL3_allTx,"
                  "OverheadPct_orig,OverheadPct_all\n";
